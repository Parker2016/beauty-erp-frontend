...
const BookingPage = () => {
  ...
  const availableSlots = ['10:00', '13:30', '15:00', '16:30', '19:00'];
          ...
          <div className="animate-fade-in-up">
              ...
              {calendarDays.map((dateObj, i) => {
                ...
                return (
                  <div key={i} className="flex justify-center">
                    <button onClick={() => !past && setSelectedDate(dateObj)} disabled={past} className={`w-10 h-10 rounded-full font-bold transition-all text-sm ${active ? 'bg-pink-400 text-white shadow-lg' : past ? 'text-gray-200 cursor-not-allowed' : 'text-gray-700 hover:bg-pink-50'}`}>{dateObj.getDate()}</button>
                  </div>
                );
              })}
            </div>
            ...
            {selectedDate && (
              <div className="mt-8 border-t border-gray-50 pt-6">
                <p className="text-gray-400 text-sm mb-4 font-medium">{selectedDate.getMonth()+1}/{selectedDate.getDate()} 可預約時間</p>
                <div className="grid grid-cols-3 gap-3">
                  {availableSlots.map(time => (
                    <button key={time} onClick={() => setSelectedTime(time)} className={`py-3 rounded-xl border-2 font-bold transition-all text-sm ${selectedTime === time ? 'border-pink-400 bg-pink-50 text-pink-500' : 'border-pink-50 text-pink-300 hover:border-pink-200'}`}>{time}</button>
                  ))}
                </div>
              </div>
            )}
      ...
      {step === 3 && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white p-5 border-t border-gray-50 flex items-center space-x-3 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
          ...
          <button 
            ...
            className={`px-8 py-4 rounded-2xl font-bold text-white transition-all ${(!selectedDate || !selectedTime) ? 'bg-gray-200' : 'bg-pink-400 shadow-lg active:scale-95'}`}
          >
            填寫預約備註 ➔
          </button>
        </div>
      ...
      {step === 4 && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white p-5 border-t border-gray-50 flex items-center space-x-3 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
          <div className="flex-1">
            ...
            <p className="text-sm font-bold text-gray-700 truncate">{selectedDate.getMonth()+1}/{selectedDate.getDate()} {selectedTime}</p>
          </div>
          <button 
            onClick={handleFinalSubmit}
            disabled={!customerData.name || !customerData.phone || isLoading}
            className={`px-8 py-4 rounded-2xl font-bold text-white transition-all ${(!customerData.name || !customerData.phone || isLoading) ? 'bg-gray-200' : 'bg-gray-900 shadow-lg active:scale-95'}`}
          >
            {isLoading ? '送出中...' : '確認預約 ➔'}
          </button>
        </div>
